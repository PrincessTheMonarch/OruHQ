...
const accountTypes = [
  {
    ...
    icon: User,
    label: "Freelancer",
    desc: "I work solo with multiple clients.",
  },
  {
    id: "agency",
    icon: Briefcase,
    label: "Small Agency",
    desc: "I work with a team of 2–10 people.",
  },
  {
    id: "side-hustler",
    icon: Zap,
    label: "Side Hustler",
    desc: "I freelance part-time alongside a job.",
  },
];
    ...
    <div className="flex gap-2 items-center mb-8">
      {steps.map((s, i) => (
        <div
          ...
          style={{
            width: i === idx ? 24 : 8,
            background: i <= idx ? "#8d0000" : "#e2e8f0",
          }}
        />
      ))}
    </div>
...
function InputField({
  ...
  return (
    <div className="flex flex-col gap-2">
      <label
        className="text-[14px]"
        style={{ fontFamily: "'Inter', sans-serif", fontWeight: 500, color: "#334155" }}
      >
        {label}
      </label>
      <input
        ...
        onFocus={(e) =>
          (e.currentTarget.style.borderColor = "#8d0000")
        }
        onBlur={(e) =>
  ...
  return (
    <Card>
      <div className="p-8">
        <div className="mb-6">
          <h1
            className="text-[24px] tracking-[-0.6px] mb-1"
            style={{ fontFamily: "'Archivo', sans-serif", fontWeight: 600, color: "#0f172a" }}
          >
            Welcome to OruHQ
          </h1>
      ...
      <div className="p-8">
        <div className="flex flex-col items-center text-center mb-6">
          <div
            ...
            style={{ background: "rgba(141,0,0,0.1)" }}
          >
            <Mail className="size-8" style={{ color: "#8d0000" }} strokeWidth={1.5} />
          </div>
          <h2
            className="text-[24px] tracking-[-0.6px] mb-2"
            style={{ fontFamily: "'Archivo', sans-serif", fontWeight: 600, color: "#0f172a" }}
          >
            Almost done!
          </h2>
        ...
        <div className="flex flex-col gap-2">
          <PrimaryBtn onClick={onNext}>Send Magic Link</PrimaryBtn>
          <button
            onClick={onBack}
  ...
  return (
    <div className="w-full max-w-[512px]">
      <div className="mb-8 text-center">
        <h2
          className="text-[30px] tracking-[-0.75px] mb-2"
      ...
      <div className="flex flex-col gap-3 mb-6">
        {accountTypes.map(({ id, icon: Icon, label, desc }) => (
          <button
            ...
            style={{
              background: "white",
              border: selected === id ? "2px solid #8d0000" : "2px solid #e5e7eb",
            }}
          >
            <div
              className="size-12 rounded-full flex items-center justify-center shrink-0"
              style={{ background: selected === id ? "rgba(141,0,0,0.1)" : "#f1f5f9" }}
            >
              <Icon
                className="size-6"
                style={{ color: selected === id ? "#8d0000" : "#64748b" }}
                strokeWidth={1.75}
              />
            </div>
            <div>
              <p
                className="text-[18px] tracking-[-0.45px]"
                style={{ fontFamily: "'Archivo', sans-serif", fontWeight: 700, color: "#0f172a" }}
              >
                {label}
              </p>
    ...
    <Card>
      <div className="p-8">
        <div className="flex flex-col items-center text-center mb-6">
          <div
            className="size-16 rounded-full flex items-center justify-center mb-5"
            style={{ background: "rgba(0,141,110,0.1)" }}
          >
        ...
        <div className="relative mb-2">
          ...
          {open && (
            <div
              className="absolute top-full left-0 right-0 mt-1 rounded-xl shadow-lg overflow-hidden z-10"
...
function LoadingStep() {
  ...
  return (
    <div className="flex flex-col items-center gap-6">
      <span
        className="text-[40px] tracking-[-0.6px]"
        style={{ fontFamily: "'Archivo Black', sans-serif", color: "#f1f5f9" }}
      >
        OruHQ
      </span>
      <p
...
export default function OnboardingFlow() {
  ...
  return (
    <div
      ...
      style={{
        background: step === "loading" ? "#8d0000" : "#f6f2ec",
        transition: "background 0.4s",
      }}
    >
      {/* Back to landing — hidden during loading */}
      {step !== "loading" && (
        <button
          onClick={() => navigate("/")}
      ...
      {step === "currency" && (
        <div className="flex flex-col items-center gap-6 w-full max-w-[448px]">
          ...
          <CurrencyStep
            onNext={(currency, symbol) => {
              setUserData((u) => {
                const finalUser = { ...u, currency, symbol };
                setUser(finalUser);
                return finalUser;
              });
              setStep("loading");
